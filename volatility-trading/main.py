...
if __name__ == '__main__':
    keys_list = ["symbol", "market_cap_rank", "price_change_percentage_24h", "market_cap_change_percentage_24h"]
    cg = CoinGeckoAPI()

    top_100_coins = []
    for i in range(1, 5):
        top_100_coins.extend(cg.get_coins_markets(vs_currency='usd', page=i))
    top_100_symbols = [coin['symbol'].upper() for coin in top_100_coins]

    signal_coins = {}
    pairs = {line.strip() + PAIR_WITH for line in open(TICKERS) if
             line.strip() in top_100_symbols and line.strip() not in STABLECOINS}
